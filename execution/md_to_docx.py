"""
Конвертер Markdown -> Word (.docx) для курса «Силовая электроника».
...
Скрипт:
1) конвертирует все Лекция.md и Лабораторная.md в Результаты/ (.docx);
2) собирает единую методичку из всех лекций (Методичка_Силовая_электроника.docx).
"""
...
COURSE_DIR = ROOT / "Силовая_электроника"
...
def add_runs(paragraph, text: str) -> None:
    ...
    # Разбиваем по токенам жирного и инлайн-кода, сохраняя разделители
...
def main() -> None:
    ...
    for folder in folders:
        ...
        lab = folder / "Лабораторная.md"
        if lecture.exists():
            convert_file(lecture, RESULT_DIR / f"Лекция_{num}_{title}.docx")
        if lab.exists():
            convert_file(lab, RESULT_DIR / f"Лабораторная_{num}_{title}.docx")

    # 3) Единая методичка из лекций
    build_methodichka(folders)
    print("Готово.")


def build_methodichka(folders: list[Path]) -> None:
    """Собрать единую методичку по курсу из всех лекций."""
    doc = Document()
    set_base_style(doc)
    ...
    run = title.add_run("СИЛОВАЯ ЭЛЕКТРОНИКА")
    run.bold = True
    run.font.size = Pt(28)
    sub = doc.add_paragraph()
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r2 = sub.add_run("Методическое пособие (курс лекций)")
    r2.font.size = Pt(16)
    meta = doc.add_paragraph()
    meta.alignment = WD_ALIGN_PARAGRAPH.CENTER
    meta.add_run("Бакалавриат · 17 лекций · теория, расчёты, моделирование")
    doc.add_page_break()

    # Содержание
    doc.add_heading("Содержание", level=1)
    lecture_titles: list[tuple[str, str]] = []
    for folder in folders:
        num, title_txt = folder_label(folder.name)
        lecture = folder / "Лекция.md"
        if lecture.exists():
            first = lecture.read_text(encoding="utf-8").splitlines()[0]
            heading = first.lstrip("# ").strip()
            lecture_titles.append((num, heading))
            p = doc.add_paragraph(style="List Number")
            p.add_run(heading)
    doc.add_page_break()

    # Тело: все лекции подряд, каждая с новой страницы
    ...
    out = RESULT_DIR / "Методичка_Силовая_электроника.docx"
    doc.save(out)
    print(f"  * {out.name} (лекций: {len(lecture_titles)})")
